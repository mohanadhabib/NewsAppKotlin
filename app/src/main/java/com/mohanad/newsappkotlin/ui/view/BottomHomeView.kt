...
fun BottomHomeView(viewModel: BottomHomeViewModel , navController: NavHostController){
        ConstraintLayout (
            ...
            val searchedNews by viewModel.getSearchedNews(viewModel.searchTxt,newsByCategory?.articles ?: emptyList()).observeAsState()
            val timeOfTopTile by viewModel.getTimeElapsed(
                latestNews?.articles?.get(0)?.publishedAt ?: ""
            ).observeAsState()
            val currentList = if (viewModel.searchTxt.isEmpty()) newsByCategory?.articles else searchedNews
            ...
            TrendingNewsTile(
                ...
                onLongPress = {
                    ...
                    if (!viewModel.isContainNews(newItem, savedNews ?: emptyList())) {
                        ...
                        Toast.makeText(context, "Article is already saved", Toast.LENGTH_SHORT)
                            .show()
                    }
                },
                onMoreClick = {
                    val time = timeOfTopTile
                    val news = gson.toJson(latestNews?.articles?.get(0)!!.copy(category = viewModel.category))
                ...
                val url = latestNews?.articles?.get(0)?.url
                ...
                items(currentList ?: emptyList()) { item ->
                    NewsTile(
                        ...
                        onLongPress = {
                            ...
                            if (!viewModel.isContainNews(newItem, savedNews ?: emptyList())) {
                                viewModel.insertNews(newItem, context)
                                Toast.makeText(context, "News article saved", Toast.LENGTH_SHORT)
                                    .show()
                            } else {
                                Toast.makeText(
                                    context,
